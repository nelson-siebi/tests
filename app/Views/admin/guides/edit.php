...
<body class="bg-gray-100 font-sans antialiased min-h-screen flex flex-col md:flex-row">
    ...
    <main class="flex-1 md:ml-72 p-4 md:p-10 pb-24 md:pb-10">
        <div class="max-w-4xl mx-auto">
            ...
            <form action="/admin/guides/edit" method="POST" enctype="multipart/form-data" class="space-y-10">
                ...
                <div class="bg-white p-8 rounded-[2rem] shadow-sm border border-slate-100 space-y-6">
                    ...
                    <div class="space-y-2">
                        <label
                            ...
                            Courte</label>
                        <input type="text" name="description" value="<?= htmlspecialchars($guide['description']) ?>"
                            class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-slate-900 font-bold focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all">
                    </div>
                    ...
                    <div class="space-y-2">
                        ...
                        <textarea name="content" required rows="4"
                            class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-slate-900 font-semibold focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all"><?= htmlspecialchars($guide['content']) ?></textarea>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="space-y-2">
                            <label
                                ...
                                de Couverture (Laisser vide pour conserver)</label>
                            <div class="flex items-center space-x-4 mb-2">
                                <img src="<?= htmlspecialchars($guide['image_url']) ?>"
                                    class="w-16 h-16 object-cover rounded-xl border border-slate-200">
                                <span class="text-xs text-slate-400 font-medium italic">Vignette actuelle</span>
                            </div>
                        ...
                        <div class="space-y-2">
                            <label class="block text-xs font-black text-slate-400 uppercase tracking-widest px-1">Ordre
                                d'affichage</label>
                            <input type="number" name="order_index" value="<?= $guide['order_index'] ?>"
                                class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-slate-900 font-bold focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all">
                        </div>
                    </div>
                    ...
                    <div class="space-y-2">
                        ...
                        <select name="status"
                            class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-slate-900 font-bold focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all">
                            <option value="active" <?= $guide['status'] === 'active' ? 'selected' : '' ?>>Actif
                            </option>
                            <option value="inactive" <?= $guide['status'] === 'inactive' ? 'selected' : '' ?>>Inactif
                            </option>
                        </select>
                    </div>
                </div>
                ...
                <div class="space-y-6">
                    ...
                    <div id="steps-container" class="space-y-6">
                        <?php foreach ($steps as $index => $step): ?>
                            <div
                                class="step-card bg-white p-8 rounded-[2rem] shadow-sm border border-slate-100 space-y-6 relative">
                                ...
                                <div class="flex items-center space-x-3 mb-2">
                                    ...
                                    <h3 class="text-lg font-bold text-slate-900">Étape <?= $index + 1 ?></h3>
                                </div>

                                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    <div class="space-y-2">
                                        <label
                                            class="block text-[10px] font-black text-slate-400 uppercase tracking-widest px-1">Titre
                                            de l'étape (Optionnel)</label>
                                    ...
                                    <div class="space-y-2">
                                        <label
                                            class="block text-[10px] font-black text-slate-400 uppercase tracking-widest px-1">Type
                                            de média</label>
                                        <select name="steps[<?= $index ?>][media_type]"
                                            class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-slate-900 font-bold focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all">
                                            <option value="none" <?= $step['media_type'] === 'none' ? 'selected' : '' ?>>
                                                Aucun</option>
                                            <option value="image" <?= $step['media_type'] === 'image' ? 'selected' : '' ?>>
                                                Image</option>
                                            <option value="video" <?= $step['media_type'] === 'video' ? 'selected' : '' ?>>
                                                Vidéo</option>
                                        </select>
                                    </div>
                                </div>
                                ...
                                <div class="space-y-2">
                                    <label
                                        class="block text-[10px] font-black text-slate-400 uppercase tracking-widest px-1">Lien
                                        du média (URL)</label>
                                    <input type="text" name="steps[<?= $index ?>][media_url]"
                                        value="<?= htmlspecialchars($step['media_url']) ?>"
                                        class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-slate-900 font-bold focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all"
                                        placeholder="https://...">
                                </div>
                                ...
                                <div class="space-y-2">
                                    <label
                                        class="block text-[10px] font-black text-primary uppercase tracking-widest px-1">OU
                                        Télécharger le média (Image/Vidéo)</label>
                            ...
                            <div class="space-y-2">
                                <label
                                    class="block text-[10px] font-black text-slate-400 uppercase tracking-widest px-1">Instructions
                                    / Détails</label>
                                <textarea name="steps[<?= $index ?>][content]" required rows="4"
                                    class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-slate-900 font-semibold focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all"><?= htmlspecialchars($step['content']) ?></textarea>
                            </div>
                        </div>
                    <?php endforeach; ?>
                </div>
    ...
    <script>
        ...
        function updateStepNumbers() {
            ...
            labels.forEach((label, index) => {
                ...
                if (headers[index]) headers[index].textContent = 'Étape ' + (index + 1);
            });
        }
